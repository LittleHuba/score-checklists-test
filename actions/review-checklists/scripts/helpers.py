...
# Marker prefix used to identify bot-managed checklist comments.
...
def make_checklist_comment_body(checklist: dict) -> str:
    """Build the Markdown body for a checklist PR comment."""
...
def find_existing_checklist_comments(pr: PullRequest) -> dict[str, Any]:
    """Find existing bot-managed checklist comments on the PR.

    Returns a dict mapping checklist-id → comment object.
    """
    result = {}
...
def find_ok_replies(
    ...
    """Find valid OK reply comments for a given checklist comment.

    We look at issue comments that were posted *after* the checklist comment
    and contain the OK marker or the OK keyword as a direct reply.
    """
    ok_replies = []
    ok_marker = OK_MARKER.format(checklist_id=checklist_id)
    # GitHub issue comments don't have a native "reply-to" — we track them
    # by looking for the OK marker that our bot edits in, or a raw OK keyword
    # posted after the checklist comment.
    for comment in pr.get_issue_comments():
        body = (comment.body or "").strip()
        if ok_marker in body:
            ok_replies.append(comment)
        elif body.upper() == OK_KEYWORD and comment.id != checklist_comment_id:
