...
An acknowledgement is a reply to a checklist comment that contains the
``OK`` keyword.  This script:

1. Enumerates relevant checklists for the PR.
2. For each checklist, finds the bot-posted comment and its OK replies.
...
def _collect_ok_acknowledgements(
    ...
    a checklist if:
      ...
      - It is a bare ``OK`` keyword posted *after* the checklist comment
        (heuristic: we assign bare OKs to the most-recently-posted checklist
        comment that precedes them).

    When a bare OK is found and successfully associated with a checklist,
    the script **edits** the comment to append the invisible marker.  This
    makes future identification deterministic and avoids ambiguity.
    """
    acks: dict[str, set[str]] = {cid: set() for cid in relevant_ids}

    # Build an ordered list of (comment_id, checklist_id) for reference.
    ...
    # Keep track of which checklist comment was most recently seen.
    last_checklist_id: str | None = None

    for comment in all_comments:
        body = (comment.body or "").strip()

        # If this comment IS a checklist comment, note it.
        if comment.id in cl_comment_ids:
            last_checklist_id = cl_comment_ids[comment.id]
            continue
        ...
        if body.upper() == OK_KEYWORD:
            # Associate with the most-recently-posted checklist comment.
            if last_checklist_id and last_checklist_id in relevant_ids:
                acks[last_checklist_id].add(user)
                # Tag the comment with the marker so future runs are
                # deterministic (avoid re-association if comment order
                # changes or new checklists are added).
                marker = OK_MARKER.format(checklist_id=last_checklist_id)
                try:
                    comment.edit(f"{body}\n{marker}")
                    print(
                        f"Tagged bare OK comment {comment.id} from {user} "
                        f"with marker for checklist '{last_checklist_id}'"
                    )
