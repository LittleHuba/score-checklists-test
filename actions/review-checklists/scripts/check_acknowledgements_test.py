...
class TestCollectOkAcknowledgements:
    def test_marker_based_ack(self):
        ...
        checklist_comment = _make_comment(
            100,
            "<!-- review-checklist:api-review --> body",
            "bot",
        ...
        pr = MagicMock()
        pr.get_issue_comments.return_value = [checklist_comment, ok_comment]

        existing = {"api-review": checklist_comment}
        acks = _collect_ok_acknowledgements(pr, existing, ["api-review"])
        assert "alice" in acks["api-review"]

    def test_bare_ok_associated_to_preceding_checklist(self):
        """A bare 'OK' is assigned to the most-recently-seen checklist."""
        checklist_comment = _make_comment(
            100,
            "<!-- review-checklist:api-review --> body",
            "bot",
    ...
    def test_bare_ok_not_matched_without_preceding_checklist(self):
        """A bare 'OK' before any checklist comment is ignored."""
        ok_comment = _make_comment(
            99,
            "OK",
            "bob",
        ...
        pr = MagicMock()
        pr.get_issue_comments.return_value = [ok_comment, checklist_comment]
    ...
    def test_multiple_reviewers(self):
        ...
        pr = MagicMock()
        pr.get_issue_comments.return_value = [cl, ok1, ok2]

        existing = {"api-review": cl}
        acks = _collect_ok_acknowledgements(pr, existing, ["api-review"])
    ...
    def test_unrelated_comment_ignored(self):
        ...
        pr = MagicMock()
        pr.get_issue_comments.return_value = [cl, normal]
...
class TestCheckAcknowledgementsMain:
    ...
    def test_no_approvers_sets_pending(
        ...
        cl_comment = _make_comment(
            100, "<!-- review-checklist:api-review -->"
        )
        pr.get_issue_comments.return_value = [cl_comment]
        mock_repo_pr.return_value = (repo, pr)
    ...
    def test_all_acked_sets_success(
        ...
        pr.get_issue_comments.return_value = [cl_comment, ok_comment]
        mock_repo_pr.return_value = (repo, pr)
