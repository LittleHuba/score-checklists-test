...
def _find_ok_comments_for_checklist(
    ...
    last_cl_comment_id: int | None = None
    for comment in all_comments:
        body = (comment.body or "").strip()

        # Track if this is a checklist comment.
        if comment.id == checklist_comment_id:
            last_cl_comment_id = comment.id
            continue
        ...
        # Bare OK keyword right after the checklist comment.
        if (
            body.upper() == OK_KEYWORD
            and last_cl_comment_id == checklist_comment_id
        ):
            ok_comments.append(comment)
...
def handle_synchronize(pr: Any) -> None:
    ...
    for cl in affected:
        ...
        comment = existing[cid]
        ok_comments = _find_ok_comments_for_checklist(pr, cid, comment.id)
